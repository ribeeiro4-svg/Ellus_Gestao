...
export function useFinanceiro() {
  ...
  const fetch = useCallback(async () => {
    if (!tenantId) {
      // Tenta carregar do localStorage primeiro para persistência em Demo
      const local = localStorage.getItem('acprobec_financeiro_demo')
      if (local) {
        setLancamentos(JSON.parse(local))
      } else {
        const mock = [
        ...
        setLancamentos(mock)
        localStorage.setItem('acprobec_financeiro_demo', JSON.stringify(mock))
      }
      setLoading(false)
      return
    }
  ...
  // Sync back to localStorage for demo persistence
  useEffect(() => {
    if (!tenantId && !loading && lancamentos.length > 0) {
      localStorage.setItem('acprobec_financeiro_demo', JSON.stringify(lancamentos))
    }
  }, [lancamentos, tenantId, loading])
