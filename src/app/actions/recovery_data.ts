...
export async function restoreAssociateConfigsAction() {
  ...
  try {
    ...
    for (const line of lines) {
      // Tenta split por TAB primeiro, se não encontrar tenta Regex para múltiplos espaços
      let parts = line.split('\t')
      if (parts.length < 5) {
        parts = line.split(/\s{2,}/) // Mínimo 2 espaços para evitar split em nomes compostos
      }
      ...
      let targetId = assoc?.id

      if (!targetId) {
        // Se não achou por email, tenta por nome exato
        const { data: assocByName } = await supabase
          .from('associados')
          .select('id')
          .eq('tenant_id', tenantId)
          .ilike('nome', nome)
      ...
      if (targetId) {
        ...
        if (!updErr) updated++
          ...
          console.error(`Erro ao atualizar ${nome}:`, updErr)
          errors++
        }
      } else {
        console.warn(`Não encontrado: ${nome} (${email})`)
        errors++
      }
    }
  } catch (err: any) {
  ...
  return { message: `Processado: ${updated} atualizados com sucesso, ${errors} não encontrados ou com erro.` }
}
