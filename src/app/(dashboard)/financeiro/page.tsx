...
export default function FinanceiroPage() {
  ...
  const filteredLancamentos = useMemo(() => {
    return lancamentos.filter(item => {
      ...
      return (filterMonth === -1 || d.getMonth() === filterMonth) && d.getFullYear() === filterYear && (filterTipo === 'todos' || item.tipo === filterTipo) && (item.descricao.toLowerCase().includes(searchTerm.toLowerCase()) || item.categoria.toLowerCase().includes(searchTerm.toLowerCase()))
    })
  }, [lancamentos, filterYear, filterMonth, filterTipo, searchTerm])
  ...
  const columns = [
    ...
    { header: 'Descrição', key: 'descricao', render: (i: any) => (<div className="flex flex-col"><div className="flex items-center gap-2"><span className="text-sm font-bold text-slate-800">{i.descricao}</span>{i.banco_transacao_id && <span className="text-[9px] font-black bg-blue-50 text-blue-500 px-1.5 py-0.5 rounded border border-blue-100 flex items-center gap-1"><RefreshCw size={8} /> OFX</span>}</div><div className="flex gap-2"><span className="text-[10px] text-slate-400 font-bold uppercase tracking-tight">{i.categoria}</span>{i.recorrencia_ativa && <span className="text-[9px] font-bold text-purple-600 bg-purple-50 px-1 rounded uppercase tracking-tighter">RECORRENTE</span>}</div></div>) },
    { header: 'Conta', key: 'conta_id', render: (i: any) => <span className="text-[11px] font-bold text-slate-400 uppercase">{contas.find(ca => ca.id === i.conta_id)?.nome || '--'}</span> },
    { header: 'Tipo', key: 'tipo', render: (i: any) => <span className={`status-badge ${i.tipo === 'receita' ? 'status-ativo' : 'status-inadimplente'}`}>{i.tipo === 'receita' ? '↑ Receita' : '↓ Despesa'}</span> },
    ...
    { header: '', key: 'acoes', className: 'text-right', render: (i: any) => (<div className="flex items-center justify-end gap-2 group-hover:opacity-100 opacity-0"><button onClick={() => { setEditingItem(i); setIsModalOpen(true) }} className="p-1.5 text-blue-600 bg-blue-50 rounded-lg hover:bg-blue-100"><Pencil size={14} /></button><button onClick={() => confirm('Excluir?') && remover(i.id)} className="p-1.5 text-red-600 bg-red-50 rounded-lg hover:bg-red-100"><XCircle size={14} /></button></div>) }
  ]
  ...
  const modalFields: Field[] = useMemo(() => [
    ...
    { name: 'selected_associados', label: 'Selecionar Associados', type: 'info', showIf: (f: any) => f.is_lote, render: (formData, handleChange) => (
      <div className="grid grid-cols-2 gap-2 mt-2 max-h-[150px] overflow-y-auto p-2 bg-slate-50 rounded-xl border border-slate-100">{associados.filter(a => a.status === 'ativo').map(a => (<label key={a.id} className="flex items-center gap-2 p-1 hover:bg-white rounded cursor-pointer text-[10px] font-bold"><input type="checkbox" checked={(formData.selected_associados || []).includes(a.id)} onChange={e => handleChange('selected_associados', e.target.checked ? [...(formData.selected_associados || []), a.id] : (formData.selected_associados || []).filter((id: string) => id !== a.id))} /><span>{a.nome}</span></label>))}</div>
    )},
    { name: 'troco_via_pix', label: 'Troco em PIX?', type: 'checkbox', showIf: (f: any) => f.tipo === 'receita' },
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <ChartCard title="📊 Fluxo Mensal" subtitle="Realizado vs Projetado"><Chart type="bar" data={{ labels: MESES, datasets: [{ label: 'Receita Real', data: chartData.recReal, backgroundColor: '#10b981', borderRadius: 4, stack: '0' }, { label: 'Receita Prov.', data: chartData.recProv, backgroundColor: 'rgba(16,185,129,0.25)', borderRadius: 4, stack: '0' }, { label: 'Desp. Real', data: chartData.despReal, backgroundColor: '#f43f5e', borderRadius: 4, stack: '1' }, { label: 'Desp. Prov.', data: chartData.despProv, backgroundColor: 'rgba(244,63,94,0.25)', borderRadius: 4, stack: '1' }] }} options={{ responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom', labels: { boxWidth: 8, font: { size: 10, weight: 'bold' } } } } }} /></ChartCard>
        <ChartCard title="📈 Acumulado Projetado" subtitle="Evolução do caixa"><Line data={{ labels: MESES, datasets: [{ label: 'Saldo Acumulado (R$)', data: chartData.recReal.map((v, i) => v), borderColor: '#6366f1', backgroundColor: 'rgba(99,102,241,0.05)', fill: true, tension: 0.4 }] }} options={{ responsive: true, maintainAspectRatio: false }} /></ChartCard>
      </div>
