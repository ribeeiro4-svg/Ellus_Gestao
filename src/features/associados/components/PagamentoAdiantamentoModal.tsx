...
interface PagamentoAdiantamentoModalProps {
  adiantamento: Adiantamento
  onClose: () => void
  onSuccess: (formaPagamento: string, parcelas: number, contaId: string, dataInicio: string) => Promise<void>
}

export default function PagamentoAdiantamentoModal({ adiantamento, onClose, onSuccess }: PagamentoAdiantamentoModalProps) {
  ...
  const valorParcela = adiantamento.valor / (modo === 'INTEGRAL' ? 1 : parcelasPagamento || 1)

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95">
        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
          <h3 className="font-black text-slate-800 text-lg">Realizar Pagamento</h3>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600">✕</button>
        </div>
        
        <div className="p-6 space-y-4">
          <div className="p-4 bg-emerald-50 rounded-xl border border-emerald-100 text-center">
            <p className="text-emerald-800 font-bold text-sm uppercase mb-1">Valor a Pagar</p>
            <p className="text-emerald-600 font-black text-2xl">{fmtR(adiantamento.valor)}</p>
          </div>
          ...
          {modo === 'PARCELADO' && (
            <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-3">
              ...
              <p className="text-sm font-medium text-slate-600 text-center">
                Serão gerados {parcelasPagamento || 0} pagamentos mensais de <strong className="text-slate-800">{fmtR(valorParcela)}</strong>
              </p>
            </div>
          )}
        </div>
