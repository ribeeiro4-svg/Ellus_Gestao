...
interface PreviewMensalidadesModalProps {
  isOpen: boolean
  onClose: () => void
  previewData: any[]
  associados?: any[]
...
export default function PreviewMensalidadesModal({ isOpen, onClose, previewData, associados, onConfirm }: PreviewMensalidadesModalProps) {
  ...
  const handleConfirm = async () => {
    if (selectedIds.size === 0) {
      alert('Selecione pelo menos um associado para lançar a mensalidade.')
      return
    }
      ...
      <div className="bg-white rounded-3xl w-full max-w-3xl max-h-[85vh] flex flex-col shadow-2xl border border-slate-100 overflow-hidden">
        ...
        <div className="flex items-center justify-between p-6 border-b border-slate-100 bg-slate-50/50">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-black text-slate-800 tracking-tight">Lançar Mensalidades</h2>
              <p className="text-sm font-bold text-slate-400">
                Selecione os associados que receberão o lançamento da taxa de mensalidade
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-slate-50/30">
          {previewData.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-slate-500 font-medium">Não há novos associados pendentes de taxa de mensalidade.</p>
            </div>
          ) : (
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                {previewData.map((item, idx) => {
                  ...
                  // Extrai o nome da descrição "MENSALIDADE - NOME"
