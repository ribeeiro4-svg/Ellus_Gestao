      ...
      <div className="relative z-10 flex flex-col h-full gap-5">
        <div className="flex items-start justify-between">
          <div className="flex flex-col gap-2">
            <p className="text-[13px] font-black uppercase tracking-[3px] text-sky-400/80">Fechamento com Ação</p>
            <h2 className="text-5xl font-black text-white tracking-tight">Decisões & Próximos Passos</h2>
            <p className="text-base text-white/50 font-semibold mt-1">Pontos que exigem deliberação da diretoria</p>
          </div>
          <div className="flex items-center gap-3 px-5 py-2 rounded-full bg-sky-500/10 border border-sky-500/20">
            ...
            <span className="text-[12px] font-black text-sky-400 uppercase tracking-wider">{decisoes.length} itens</span>
          </div>
        </div>

        {/* Lista de decisões */}
        <div className="flex flex-col gap-3 flex-1 overflow-y-auto pr-2 mt-4">
          {decisoes.map((d, i) => (
            <div key={i} className="group flex items-center gap-4 p-5 rounded-[20px] bg-white/5 border border-white/10 hover:bg-white/10 transition-all">
              <div className="w-8 h-8 rounded-xl bg-sky-500/15 border border-sky-500/20 flex items-center justify-center text-[13px] font-black text-sky-400 shrink-0">
                {i + 1}
              </div>
              {editingIdx === i ? (
                <input
                  className="flex-1 bg-transparent text-white/90 text-base font-bold outline-none border-b border-white/30 pb-1"
                  value={d}
                ...
                <p
                  className="flex-1 text-base font-bold text-white/80 cursor-text"
              ...
              <button
                ...
                className="opacity-0 group-hover:opacity-100 w-9 h-9 rounded-xl bg-rose-500/10 border border-rose-500/20 flex items-center justify-center text-rose-400/80 hover:text-rose-400 hover:bg-rose-500/20 transition-all shrink-0"
              >
                <Trash2 size={16} />
              </button>
            </div>
        ...
        <div className="flex items-center gap-4 p-5 rounded-[20px] bg-white/5 border border-white/10 mt-2">
          <input
            ...
            className="flex-1 bg-transparent text-base font-bold text-white/80 placeholder:text-white/30 outline-none"
          />
          <button
            onClick={addDecisao}
            className="w-10 h-10 rounded-xl bg-sky-500/20 border border-sky-500/30 flex items-center justify-center text-sky-400 hover:bg-sky-500/30 transition-all"
          >
            <Plus size={20} />
          </button>
        </div>

        <p className="text-[12px] text-white/40 font-bold text-center mt-2">
