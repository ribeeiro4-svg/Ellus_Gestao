...
export default function NFSeModuleTab({ nfseHook }: { nfseHook: any }) {
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in duration-700 bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
      {/* Header interno */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-black text-slate-800 tracking-tight">NFS-e: Serviços Tomados</h2>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Gestão Fiscal • Retenções • Escrituração</p>
          </div>
        </div>
      </div>

      {/* Tabs Internas */}
      <div className="flex gap-1.5 p-1.5 bg-slate-50 rounded-2xl w-fit flex-wrap border border-slate-100">
        {tabs.map(tab => (
          <button
            key={tab.id}
