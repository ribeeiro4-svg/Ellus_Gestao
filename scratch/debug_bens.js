...
async function debug() {
  ...
  console.log('Itens for NF 463625:', itens.map(i => ({ desc: i.descricao_produto, dest: i.destinacao_item })))
  
  const { data: bens } = await sb.from('bens_duraveis').select('*')
  console.log('All Bens Duraveis:', bens)
}
